...
- **Release Page:** `https://github.com/dacthinh05/AuditSoft/releases/tag/v1.1.5`

## Artifacts
1. **Windows Installer (Setup):** `AuditSoft-1.1.5-Setup.exe` (~91 MB)
   - NSIS installer, tự động tạo shortcut Desktop & Start Menu, hỗ trợ tự động nâng cấp 1-click.
2. **Windows Portable:** `AuditSoft-1.1.5-Portable.exe` (~91 MB)
...
1. Chỉnh sửa `version.json` hạ số phiên bản về bản ổn định trước đó (ví dụ `1.1.4`) kèm URL tải của bản đó.
